...
export default function SabucanCajaPage() {
  ...
  async function confirmarVentaYCanje() {
    ...
    try {
      ...
      let puntosUsados = 0;
      let puntosGanados = 0;
      let clienteFinal: SabucanClienteUi | null = null;
      ...
      if (quiereCanje) {
        const canjeRes = await fetch('/api/sabucan/canje', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            telefono: tel,
            puntos: puntosUsarNum,
            // Si también hay venta, /venta hace el sync final una sola vez
      ...
      if (hayVenta) {
        ...
        const ventaRes = await fetch('/api/sabucan/venta', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      {phase === 'done' && resultado ? (
        ...
        <div className="space-y-5">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center rounded-xl bg-[#F2691F] px-5 py-3.5 text-sm font-bold text-white hover:opacity-90"
                >
                  Sí
                </button>
                <button
          ...
          {phase === 'points_amount' && clienteFound ? (
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
              <label className={sabucanLabelClass()} htmlFor="pts-usar">
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center rounded-xl bg-[#F2691F] px-5 py-3.5 text-sm font-bold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Continuar
                </button>
              </div>
          ...
          {phase === 'sale' ? (
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-7">
              ...
              {usePoints === 'yes' && ticketNum > 0 && puntosUsarNum > 0 ? (
                <p className="mt-3 rounded-2xl border border-[#F2691F]/30 bg-[#F2691F]/10 px-4 py-3 text-sm text-white/85">
                  Total: {formatMxn(ticketNum)} − {formatMxn(descuentoMxn)} en puntos ={' '}
                  <strong className="text-[#F2691F]">{formatMxn(aPagar)} a pagar</strong>
                </p>
              ) : null}
