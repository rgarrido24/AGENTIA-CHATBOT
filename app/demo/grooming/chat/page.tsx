...
const CHIPS_STAFF = [
  ...
  '¿Qué servicio es el más pedido?',
  'Resume reglas de domicilio',
  'Mensaje para dueño con perro nervioso',
];

const CHIPS_CLIENTE = [
  '¿Cuánto cuesta el baño?',
  '¿Hacen servicio a domicilio?',
  'Quiero agendar para mi perro',
...
function ChatInner() {
  ...
  const sendMessage = useCallback(
    async (text: string, history: ChatMsg[]) => {
      ...
      try {
        const res = await fetch('/api/demo/grooming/chat', {
          method: 'POST',
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-10rem)] min-h-[420px]">
      <div className="flex gap-2 mb-4">
        ...
        <button
          ...
          style={mode === 'cliente' ? { background: '#ea580c' } : undefined}
        >
          Modo Cliente
        </button>
      </div>
      ...
      <div
        className={`flex-1 overflow-y-auto rounded-xl border p-4 space-y-3 ${
          isCliente ? 'bg-[#0b141a] border-orange-900/40' : 'bg-white/[0.03] border-white/10'
        }`}
      >
        {messages.length === 0 && (
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border ${
                  isCliente
                    ...
                    : 'border-orange-500/40 text-orange-200 hover:bg-orange-900/20'
                }`}
              >
                {s}
              </button>
        ...
        {messages.map((m, i) => (
          <div key={`${i}-${m.role}`} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              ...
              style={m.role === 'user' ? { background: isCliente ? '#ea580c' : ACCENT } : undefined}
            >
              {m.content ||
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          style={{ background: isCliente ? '#ea580c' : ACCENT }}
        >
