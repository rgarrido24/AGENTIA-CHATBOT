...
import { GroomingProvider } from './grooming-context';

const ACCENT = '#f97316';
const ACCENT_SOFT = 'rgba(249, 115, 22, 0.2)';

const NAV = [
  { href: '/demo/grooming', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/demo/grooming/agenda', label: 'Agenda', icon: CalendarDays },
  { href: '/demo/grooming/mascotas', label: 'Mascotas', icon: PawPrint },
  { href: '/demo/grooming/servicios', label: 'Servicios', icon: Scissors },
...
const TITLE_MAP: Record<string, string> = {
  '/demo/grooming': 'Dashboard',
  '/demo/grooming/agenda': 'Agenda',
  '/demo/grooming/mascotas': 'Mascotas',
  '/demo/grooming/servicios': 'Servicios',
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  const sectionTitle = pathname.startsWith('/demo/grooming/mascotas')
    ? 'Mascotas'
    : TITLE_MAP[pathname] ?? 'Grooming';
    ...
    <div className={`min-h-screen flex transition-colors ${bg}`}>
      ...
      >
        <div className={`p-4 border-b ${light ? 'border-zinc-200' : 'border-white/10'} flex items-center justify-between lg:block`}>
          <div className="flex items-center gap-2">
            ...
            >
              <PawPrint className="w-6 h-6" style={{ color: ACCENT }} />
            </div>
            <div>
              <p className="font-semibold text-sm leading-tight">Agentia Grooming</p>
              <p className={`text-xs ${muted}`}>Patitas Felices</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV.map(({ href, label, icon: Icon }) => {
            const active = pathname === href || (href !== '/demo/grooming' && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
                ...
                style={
                  active
                    ? { background: ACCENT, boxShadow: `0 8px 24px rgba(249,115,22,0.35)` }
                    : undefined
                }
              >
        ...
        <div className={`p-4 border-t ${light ? 'border-zinc-200' : 'border-white/10'} flex items-center gap-3`}>
          ...
          <p className={`text-sm truncate ${light ? 'text-zinc-700' : 'text-slate-300'}`}>
            <span className="font-medium">Valeria</span>
            <span className={muted}> · </span>
            <span className={muted}>Groomera</span>
          </p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className={`sticky top-0 z-30 flex items-center gap-3 px-4 py-3 border-b backdrop-blur-md ${headerBg}`}>
          ...
          >
            {light ? <Moon className="w-5 h-5" /> : <Sun className="w-5 h-5 text-orange-300" />}
          </button>
...
export default function GroomingShell({ children }: { children: React.ReactNode }) {
  return (
    <GroomingProvider>
