...
import { RestauranteProvider } from './restaurante-context';

const ACCENT = '#dc2626';
const ACCENT_SOFT = 'rgba(220, 38, 38, 0.25)';

const NAV = [
  { href: '/demo/restaurante', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/demo/restaurante/menu', label: 'Menú & Orden', icon: BookOpen },
  { href: '/demo/restaurante/cocina', label: 'Cocina / Bar', icon: ChefHat },
  { href: '/demo/restaurante/delivery', label: 'Delivery & CRM', icon: Bike },
...
const TITLE_MAP: Record<string, string> = {
  '/demo/restaurante': 'Dashboard',
  '/demo/restaurante/menu': 'Menú & Orden',
  '/demo/restaurante/cocina': 'Cocina / Bar',
  '/demo/restaurante/delivery': 'Delivery & CRM',
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  const sectionTitle = TITLE_MAP[pathname] ?? 'Restaurante';
      ...
      >
        <div className={`p-4 border-b ${light ? 'border-zinc-200' : 'border-white/10'} flex items-center justify-between lg:block`}>
          <div className="flex items-center gap-2">
            ...
            >
              <UtensilsCrossed className="w-6 h-6" style={{ color: ACCENT }} />
            </div>
            <div>
              <p className="font-semibold text-sm leading-tight">Agentia Restaurante</p>
              <p className={`text-xs ${muted}`}>La Séptima — Bar & Kitchen</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV.map(({ href, label, icon: Icon }) => {
            const active =
              pathname === href || (href !== '/demo/restaurante' && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
                ...
                style={
                  active
                    ? { background: ACCENT, boxShadow: `0 8px 24px ${ACCENT}44` }
                    : undefined
                }
              >
...
export default function RestauranteShell({ children }: { children: React.ReactNode }) {
  return (
    <RestauranteProvider>
