...
const CHIPS_STAFF = [
  ...
  '¿Qué especialista tiene más carga esta semana?',
  'Dame un mensaje para recordar cita por WhatsApp',
  'Sugiere upsell para cliente Regular',
  'Resume ingresos por categoría de servicio',
];
...
function ChatInner() {
  ...
  const sendMessage = useCallback(
    async (text: string, history: ChatMsg[]) => {
      ...
      try {
        const res = await fetch('/api/demo/spa/chat', {
          method: 'POST',
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-10rem)] min-h-[420px]">
      ...
      {isCliente && (
        <div
          ...
          style={{ borderColor: `${PINK}55`, background: 'rgba(236,72,153,0.12)' }}
        >
          En producción este chat puede vivir en WhatsApp o la web del spa; aquí simulas la experiencia del cliente.
        </div>
      )}
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border ${
                  isCliente
                    ...
                    : 'border-violet-500/40 text-violet-200 hover:bg-violet-900/20'
                }`}
              >
                {s}
              </button>
...
export default function SpaChatPage() {
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-slate-400 text-sm max-w-3xl mx-auto">
        ...
        <span>Chat IA — Lumina Spa (demo)</span>
      </div>
