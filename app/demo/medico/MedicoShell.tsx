...
import { MedicoProvider } from './medico-context';

const ACCENT = '#16a34a';
const ACCENT_SOFT = 'rgba(22, 163, 74, 0.18)';

const NAV = [
  { href: '/demo/medico', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/demo/medico/agenda', label: 'Agenda', icon: CalendarDays },
  { href: '/demo/medico/pacientes', label: 'Pacientes', icon: Users },
  { href: '/demo/medico/expediente', label: 'Expediente', icon: FileText },
  { href: '/demo/medico/recetas', label: 'Recetas e Incapacidades', icon: Pill },
...
const TITLE_MAP: Record<string, string> = {
  '/demo/medico': 'Dashboard',
  '/demo/medico/agenda': 'Agenda',
  '/demo/medico/pacientes': 'Pacientes',
  '/demo/medico/expediente': 'Expediente',
  '/demo/medico/recetas': 'Recetas e Incapacidades',
    ...
    <div className={`min-h-screen flex transition-colors ${bg}`}>
      ...
      >
        <div className={`p-4 border-b ${light ? 'border-zinc-200' : 'border-white/10'} flex items-center justify-between lg:block`}>
          <div className="flex items-center gap-2">
            ...
            >
              <HeartPulse className="w-6 h-6" style={{ color: ACCENT }} />
            </div>
            <div>
              <p className="font-semibold text-sm leading-tight">Agentia Médico</p>
              <p className={`text-xs ${muted}`}>Centro Médico Salud+</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV.map(({ href, label, icon: Icon }) => {
            const active = pathname === href || (href !== '/demo/medico' && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
                ...
                style={
                  active
                    ? { background: ACCENT, boxShadow: `0 8px 24px rgba(22,163,74,0.35)` }
                    : undefined
                }
              >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className={`sticky top-0 z-30 flex items-center gap-3 px-4 py-3 border-b backdrop-blur-md ${headerBg}`}>
          ...
          <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-600 text-white shrink-0">DEMO</span>
        </header>
...
export default function MedicoShell({ children }: { children: React.ReactNode }) {
  return (
    <MedicoProvider>
