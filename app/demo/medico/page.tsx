...
function badgeStatus(c: Cita['status']) {
  const m: Record<Cita['status'], string> = {
    confirmada: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/25 dark:text-emerald-200',
    pendiente: 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-200',
    completada: 'bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-200',
    ...
    <>
      ...
      <section className="max-w-7xl mx-auto mb-12 flex flex-col lg:flex-row items-center gap-10 px-2">
        <div className="flex-1 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Demo en vivo · Centro Médico</p>
          <h2 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white leading-tight">
            Consultas médicas<br />
            <span className="text-emerald-600 dark:text-emerald-400">agendadas sin llamadas</span>
          </h2>
          ...
          <ul className="space-y-2 text-sm text-zinc-700 dark:text-slate-300">
            {['Agendamiento por especialidad', 'Precios orientativos incluidos', 'Nunca da diagnósticos — siempre remite al médico'].map(t => (
              <li key={t} className="flex items-center gap-2"><span className="text-emerald-600 dark:text-emerald-400">✓</span>{t}</li>
            ))}
          </ul>
        </div>
      ...
      <p className="text-zinc-500 dark:text-slate-500 text-sm">{BRAND_MEDICO.nombre}</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { label: 'Citas hoy', value: hoy.length, color: 'text-emerald-600 dark:text-emerald-300' },
          { label: 'Pacientes activos', value: activos, color: 'text-green-600 dark:text-green-300' },
          { label: 'Cuentas por cobrar', value: cxp, format: fmt, color: 'text-amber-600 dark:text-amber-300' },
          { label: 'Tratamientos en curso', value: tCurso, color: 'text-lime-600 dark:text-lime-300' },
        ].map((k, i) => (
          <motion.div
            key={k.label}
      ...
      <div>
        <h2 className="text-sm font-semibold text-zinc-900 dark:text-white mb-3">Citas de hoy por médico</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {porMedico.map((col) => (
            <div key={col.id} className="rounded-xl border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03] p-4 min-h-[200px] shadow-sm dark:shadow-none">
              <p className="text-emerald-600 dark:text-emerald-400 font-medium text-sm mb-3">{col.nombre}</p>
              <p className="text-[10px] text-zinc-400 dark:text-slate-500 mb-2">{col.especialidad}</p>
                ...
                {col.citas.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="text-xs border border-zinc-100 dark:border-white/5 rounded-lg px-2 py-2 text-zinc-700 dark:text-slate-300">
                      <span className="font-mono text-emerald-600 dark:text-emerald-300">{c.hora}</span> · {p?.nombre} · {c.tipo}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl border border-amber-200 bg-amber-50 dark:border-amber-900/40 dark:bg-amber-950/20 p-4">
          ...
          <p className="text-xs text-zinc-500 dark:text-slate-400 mt-4 mb-2">Citas sin confirmar (pendientes)</p>
          <ul className="text-xs space-y-1 text-zinc-700 dark:text-slate-300">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03] p-4 h-[280px] shadow-sm dark:shadow-none">
          <h3 className="text-sm font-semibold text-zinc-800 dark:text-slate-200 mb-2">Ingresos por tipo de cita (completadas)</h3>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03] p-4 h-[300px] shadow-sm dark:shadow-none">
        <h3 className="text-sm font-semibold text-zinc-800 dark:text-slate-200 mb-2">Citas por mes (últimos 6)</h3>
