...
const CHIPS_STAFF = [
  'Tiempos de ayuno para perfil lipídico',
  'Criterios de alarma post-procedimiento (referencia)',
  'Checklist antes de ecocardiograma',
];

const CHIPS_CLIENTE = [
  '¿Cuánto cuesta una consulta general?',
  'Tengo dolor en el pecho, ¿qué hago?',
  'Quiero agendar con cardiología',
  '¿Atienden pediatría los sábados?',
];
...
function ChatInner() {
  ...
  const sendMessage = useCallback(
    async (text: string, history: ChatMsg[]) => {
      ...
      try {
        const res = await fetch('/api/demo/medico/chat', {
          method: 'POST',
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-10rem)] min-h-[420px]">
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold ${mode === 'staff' ? 'text-white' : 'bg-white/10 text-slate-400'}`}
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold ${mode === 'cliente' ? 'text-white' : 'bg-white/10 text-slate-400'}`}
          style={mode === 'cliente' ? { background: '#15803d' } : undefined}
        >
          🩺 Paciente
        </button>
      </div>
      ...
      <div
        className={`flex-1 overflow-y-auto rounded-xl border p-4 space-y-3 ${
          isCliente ? 'bg-[#0b141a] border-emerald-900/40' : 'bg-white/[0.03] border-white/10'
        }`}
      >
        {messages.length === 0 && (
            ...
            {(mode === 'staff' ? CHIPS_STAFF : CHIPS_CLIENTE).map((s) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border ${
                  isCliente ? 'border-emerald-700 text-emerald-200 hover:bg-emerald-900/40' : 'border-emerald-500/40 text-emerald-200 hover:bg-emerald-900/20'
                }`}
              >
                {s}
              </button>
        ...
        {messages.map((m, i) => (
          <div key={`${i}-${m.role}`} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              ...
              style={m.role === 'user' ? { background: isCliente ? '#15803d' : ACCENT } : undefined}
            >
              {m.content ||
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          style={{ background: isCliente ? '#15803d' : ACCENT }}
        >
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-slate-400 text-sm max-w-3xl mx-auto">
        ...
        <span>Asistente IA — Salud+ (demo)</span>
      </div>
