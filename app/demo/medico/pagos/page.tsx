...
import { HOY_MED } from '@/lib/mock-data-medico';
import { useMedico } from '../medico-context';

const fmt = (n: number) =>
  new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN', maximumFractionDigits: 0 }).format(n);

export default function MedicoPagosPage() {
  const { pacientes, consultas, pagos, registrarPago } = useMedico();
  const [modal, setModal] = useState(false);
  const [pid, setPid] = useState('pm1');
  const [monto, setMonto] = useState(500);
  const [metodo, setMetodo] = useState<'efectivo' | 'tarjeta' | 'transferencia'>('efectivo');
  const [ref, setRef] = useState('');
  const [toast, setToast] = useState<string | null>(null);

  const ingresosMes = useMemo(() => pagos.reduce((s, p) => s + p.monto, 0) + 52000, [pagos]);
  ...
  const cobrosHoy = useMemo(() => pagos.filter((p) => p.fecha === HOY_MED).reduce((s, p) => s + p.monto, 0), [pagos]);

  const cuentas = useMemo(() => {
    return consultas
      .filter((c) => c.saldo > 0)
      .map((c) => {
        const pa = pacientes.find((x) => x.id === c.pacienteId);
        const dias = 5 + (c.id.length % 20);
        return { ...c, paciente: pa, dias };
      })
      .slice(0, 15);
  }, [consultas, pacientes]);
  ...
  const totalesMetodo = useMemo(() => {
    ...
    for (const p of pagos.filter((x) => x.fecha === HOY_MED)) {
      m[p.metodo] += p.monto;
    }
    return m;
  }, [pagos]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          <div key={k.label} className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <p className="text-2xl font-bold text-emerald-300 mt-1">{k.f(k.value)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-end">
        <button type="button" onClick={() => setModal(true)} className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-semibold text-sm">
          Registrar pago
        </button>
      </div>
      ...
      <div className="rounded-xl border border-white/10 p-4">
        <h3 className="text-sm font-semibold mb-2">Pagos del día por método</h3>
        <div className="grid grid-cols-3 gap-2 text-sm">
          <p>Efectivo: {fmt(totalesMetodo.efectivo)}</p>
          <p>Tarjeta: {fmt(totalesMetodo.tarjeta)}</p>
          <p>Transferencia: {fmt(totalesMetodo.transferencia)}</p>
        </div>
        <ul className="mt-4 space-y-1 text-xs text-slate-400">
          {pagos
            .filter((x) => x.fecha === HOY_MED)
            .map((p) => (
              <li key={p.id}>
                {p.pacienteNombre} — {fmt(p.monto)} ({p.metodo})
      ...
      <AnimatePresence>
        {modal && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <motion.div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0f1624] p-6" initial={{ scale: 0.95 }} animate={{ scale: 1 }}>
              ...
              <div className="space-y-3 text-sm">
                ...
                <button type="button" onClick={registrar} className="w-full py-3 rounded-xl font-semibold bg-emerald-600 text-white">
                  Registrar
                </button>
              </div>
            </motion.div>
