...
const CHIPS_STAFF = [
  'Tiempos estimados para endodoncia vs extracción',
  'Interacción ibuprofeno y anticoagulantes (referencia)',
  'Checklist pre-implante',
];

const CHIPS_CLIENTE = [
  '¿Cuánto cuesta una limpieza?',
  'Tengo dolor de muela urgente',
...
function ChatInner() {
  ...
  const sendMessage = useCallback(
    async (text: string, history: ChatMsg[]) => {
      ...
      try {
        const res = await fetch('/api/demo/dentista/chat', {
          method: 'POST',
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-10rem)] min-h-[420px]">
      <div className="flex gap-2 mb-4">
        ...
        <button
          ...
          style={mode === 'cliente' ? { background: '#0369a1' } : undefined}
        >
          🦷 Paciente
        </button>
      </div>
            ...
            {(mode === 'staff' ? CHIPS_STAFF : CHIPS_CLIENTE).map((s) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border ${
                  isCliente ? 'border-sky-700 text-sky-200 hover:bg-sky-900/40' : 'border-sky-500/40 text-sky-200 hover:bg-sky-900/20'
                }`}
              >
                {s}
              </button>
        ...
        {messages.map((m, i) => (
          <div key={`${i}-${m.role}`} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              ...
              style={m.role === 'user' ? { background: isCliente ? '#0369a1' : ACCENT } : undefined}
            >
              {m.content ||
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          style={{ background: isCliente ? '#0369a1' : ACCENT }}
        >
