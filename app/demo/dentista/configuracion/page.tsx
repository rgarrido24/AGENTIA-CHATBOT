...
import { MOCK_RECETAS_HISTORIAL } from '@/lib/mock-data-dentista';
...
export default function DentistaConfigPage() {
  ...
  // Perfil state
  const [nombre, setNombre] = useState('Dr. Carlos Herrera');
  const [cedula, setCedula] = useState('1234567');
  const [especialidad, setEspecialidad] = useState('Odontología General');
  const [universidad, setUniversidad] = useState('UNAM — Facultad de Odontología');
  const [telefono, setTelefono] = useState('999-000-1111');
  const [email, setEmail] = useState('dr.herrera@clinica.com');
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
            ...
            {firmaBase64 ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setFirmaBase64('')} className="text-xs text-sky-400">Redibujar</button>
                  <span className="text-slate-600 text-xs">·</span>
                  <label className="text-xs text-sky-400 cursor-pointer">
                    Subir imagen
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              type="button"
              onClick={handleSave}
              className="px-5 py-2.5 rounded-xl font-semibold text-white bg-sky-600 hover:bg-sky-500 text-sm"
            >
              Guardar cambios
            </button>
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl border p-5 space-y-3 transition-all ${
                    active ? 'border-sky-500 bg-sky-500/10' : 'border-white/10 bg-white/[0.03]'
                  }`}
                >
                  {badge && (
                    <span className="text-[10px] bg-sky-600/30 text-sky-300 px-2 py-0.5 rounded font-medium">
                      {badge}
                    </span>
                  )}
                  <h3 className={`font-bold text-sm ${active ? 'text-sky-300' : 'text-white'}`}>{title}</h3>
            ...
            <tbody>
              {MOCK_RECETAS_HISTORIAL.map((r) => (
                <tr key={r.id} className="border-b border-white/5">
                  ...
                  <td className="p-3">
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-sky-600/20 text-sky-300 border border-sky-500/30">
                      {nivelLabel(1)}
                    </span>
                  </td>
                  <td className="p-3">
